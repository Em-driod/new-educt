...
import {
  ...
  FiCpu,
  FiLayers,
  FiFileText, // More generic document icon
} from 'react-icons/fi';
...
// --- UPDATED INTERFACES TO STRICTLY MATCH YOUR ORIGINAL BACKEND'S OUTPUT ---
...
interface InsightData {
  keyConcepts: SectionExplanation[];
  readingGuide: string[]; // Still a simple string array as per your backend
  relatedBooks: Book[];
  contextualExplanation: SectionExplanation[]; // Array of objects
  importantSections: SectionExplanation[];
}

interface AnalysisData {
  summary: SectionExplanation[]; // Array of objects
  keyPoints: SectionExplanation[];
  actionItems: SectionExplanation[];
  followUpQuestions: SectionExplanation[];
...
interface AnimatedSectionProps {
  title: string;
  // Content can now be a string (for a single block) or an array of strings (for lists)
  content?: string;
  contentList?: string[]; // For rendering simple string lists
  complexContentList?: SectionExplanation[] | Book[] | TermDefinition[]; // For rendering lists of objects
  color: 'lime' | 'white';
  delay?: number;
...
const AnimatedSection = ({
  ...
  content,
  contentList,
  complexContentList, // New prop for structured data
  color,
  delay = 0,
  icon,
  ...
  // Helper function to render items from complexContentList
  const renderComplexListItem = (item: any) => {
    if ('section' in item && 'explanation' in item) {
      return `**${item.section}**: ${item.explanation}`;
    } else if ('title' in item && 'author' in item) {
      return `**${item.title}** by ${item.author}`;
    } else if ('term' in item && 'definition' in item) {
      return `**${item.term}**: ${item.definition}`;
    }
    return JSON.stringify(item); // Fallback for unexpected formats
  };
  ...
  return (
    ...
    >
        ...
        <ul className="space-y-3">
          {contentList.map((item, idx) => (
            ...
            >
              ...
              <span className="text-gray-200 text-opacity-90 text-base">
                 {/* Handle markdown bolding for simple strings */}
                {item.includes('**') ? (
                  <span>
      ...
      {/* Render complex object lists */}
      {complexContentList && (
        <ul className="space-y-3">
          {complexContentList.map((item, idx) => (
            <motion.li
              key={idx}
              initial={{ opacity: 0, x: -10 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              <FiChevronRight className={`mt-1 flex-shrink-0 ${selectedColors.text}`} />
              <span className="text-gray-200 text-opacity-90 text-base">
                {/* Render the complex item using the helper */}
                {renderComplexListItem(item).includes('**') ? (
                  <span>
                    {renderComplexListItem(item).split('**').map((part, i) =>
                      i % 2 === 1 ? <strong key={i}>{part}</strong> : part
                    )}
                  </span>
                ) : (
...
export default function FileUploader({ }: FileUploaderProps) {
  ...
  const commonRequestConfig = (endpoint: string, setter: any, loaderSetter: any, tab: 'insights' | 'analysis') => async () => {
    ...
    try {
      const res = await axios.post(`https://educt-back.onrender.com/api/${endpoint}`, formData, {
        headers: {
        ...
        >
          ...
          <div className="p-8 md:p-10">
            ...
            >
              ...
              {/* Subtle Glitch Effect on Hover for Drag Zone */}
                  ...
                  <div className="p-5 bg-gray-950 rounded-xl border border-lime-500/50 flex items-center gap-5 shadow-inner relative overflow-hidden">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-400 font-light">
                        Our advanced AI is{' '}
                        {loadingAnalysis ? 'performing deep semantic analysis' : 'extracting key insights'} from your
                        data. Please await results.
                      </p>
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    {activeTab === 'insights' && insightData && (
                      <>
                        {/* Contextual Understanding - now an array of objects */}
                        <AnimatedSection
                          title="Contextual Understanding"
                          complexContentList={insightData.contextualExplanation}
                          color="lime"
                          icon={<FiBookOpen />}
                        />
                        {/* Key Concepts Identified - array of objects */}
                        <AnimatedSection
                          title="Key Concepts Identified"
                          complexContentList={insightData.keyConcepts}
                          color="lime"
                          delay={0.1}
                          icon={<FiLayers />}
                        />
                        {/* Reading Guide - still a simple string array */}
                        <AnimatedSection
                          ...
                          icon={<FiFileText />}
                        />
                        {/* Recommended Resources - array of objects */}
                        <AnimatedSection
                          title="Recommended Resources"
                          complexContentList={insightData.relatedBooks}
                          color="lime"
                          delay={0.3}
                          icon={<FiBookOpen />}
                        />
                        {/* Important Sections - array of objects */}
                        <AnimatedSection
                          title="Important Sections"
                          complexContentList={insightData.importantSections}
                          color="lime"
                          delay={0.4}
                          icon={<FiFileText />}
                        />
                    ...
                    {activeTab === 'analysis' && analysisData && (
                      <>
                        ...
                        <AnimatedSection
                          ...
                          complexContentList={analysisData.summary}
                          color="white"
                          icon={<FiZap />}
                        />
                        {/* Critical Analysis Points - array of objects */}
                        <AnimatedSection
                          title="Critical Analysis Points"
                          complexContentList={analysisData.keyPoints}
                          color="white"
                          delay={0.1}
                          icon={<FiZap />}
                        />
                        {/* Actionable Recommendations - array of objects */}
                        <AnimatedSection
                          title="Actionable Recommendations"
                          complexContentList={analysisData.actionItems}
                          color="white"
                          delay={0.2}
                          icon={<FiZap />}
                        />
                        {/* Follow-Up Questions - array of objects */}
                        <AnimatedSection
                          title="Follow-Up Questions"
                          complexContentList={analysisData.followUpQuestions}
                          color="white"
                          delay={0.3}
                          icon={<FiZap />}
                        />
                        {/* Technical Terminology Breakdown - array of objects */}
                        <AnimatedSection
                          title="Technical Terminology Breakdown"
                          complexContentList={analysisData.technicalTerms}
                          color="white"
                          delay={0.4}
                          icon={<FiCpu />}
                        />
